...
func _ready() -> void:
	...
	if not self.tile_map_layer:
		...
		self.tile_map_layer = get_node_or_null("TileMapLayer")
...
func _handle_camera_zoom(event: InputEvent) -> void:
	...
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_WHEEL_UP and event.pressed:
			self.camera.zoom = self.camera.zoom * (1.0 + self.zoom_speed)
			self.camera.zoom = self.camera.zoom.clamp(Vector2(self.min_zoom, self.min_zoom), Vector2(self.max_zoom, self.max_zoom))
...
func _handle_camera_pan(delta: float) -> void:
	...
	if _is_following_mech and self.mech_node and not self.allow_manual_pan_while_following:
		return
	...
	if pan_direction != Vector2.ZERO:
		var pan_amount: Vector2 = pan_direction.normalized() * self.pan_speed * delta / self.camera.zoom.x

		if _is_following_mech and self.allow_manual_pan_while_following:
			# Add to offset instead of moving camera directly
			_manual_camera_offset += pan_amount
		else:
			# Manual camera control (not following mech)
			self.camera.position += pan_amount

func _handle_mouse_clicks(event: InputEvent) -> void:
	if event is InputEventMouseButton:
		if event.button_index == MOUSE_BUTTON_LEFT and event.pressed:
			var world_pos: Vector2 = get_global_mouse_position()
			var hex_coords: Vector2i = self.world_to_hex(world_pos)
...
func world_to_hex(world_pos: Vector2) -> Vector2i:
	...
	return self.tile_map_layer.local_to_map(world_pos)
...
func hex_to_world(hex_coords: Vector2i) -> Vector2:
	...
	return self.tile_map_layer.map_to_local(hex_coords)
...
func get_neighbor_in_direction(hex: Vector2i, direction: int) -> Vector2i:
	...
	var dir_vec: Vector2i = HEX_DIRECTIONS[direction]
	return hex + dir_vec
...
func has_tile(hex: Vector2i) -> bool:
	...
	return self.tile_map_layer.get_cell_source_id(hex) != -1
...
func get_tile_id(hex: Vector2i) -> int:
	...
	return self.tile_map_layer.get_cell_source_id(hex)
...
func _draw() -> void:
	...
	if _current_hover_hex != Vector2i.MAX:
		var world_pos: Vector2 = self.hex_to_world(_current_hover_hex)
		var local_pos: Vector2 = to_local(world_pos)

		var text: String = "(%d, %d)" % [_current_hover_hex.x, _current_hover_hex.y]
		draw_string(ThemeDB.fallback_font, local_pos - Vector2(30, -5), text, HORIZONTAL_ALIGNMENT_CENTER, -1, 16, self.debug_text_color)

	# Draw marker on last clicked tile
	if _last_click_hex != Vector2i.MAX:
		var world_pos: Vector2 = self.hex_to_world(_last_click_hex)
		var local_pos: Vector2 = to_local(world_pos)
...
func _handle_camera_follow(delta: float) -> void:
	if not self.camera or not _is_following_mech or not self.mech_node:
		return

	if not is_instance_valid(self.mech_node):
		push_warning("HexGrid: Mech node is no longer valid")
		self.mech_node = null
		return

	var target_position: Vector2 = self.mech_node.global_position + _manual_camera_offset

	# Skip update if already close enough (sub-pixel precision)
	if self.camera.position.distance_squared_to(target_position) < 0.01:
		return

	if self.smooth_camera:
		self.camera.position = self.camera.position.lerp(target_position, self.camera_lerp_speed * delta)
	else:
...
func get_camera_position() -> Vector2:
	return self.camera.position if self.camera else Vector2.ZERO
...
func is_following_mech() -> bool:
	return _is_following_mech and self.mech_node != null
